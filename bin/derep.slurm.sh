...
# output from dedupe.sh is interleaved
errorExit "dedupe.sh failed: ${trimPEr1}, ${trimPEr2}"
echo "dedupe.sh finished sucessfully: ${trimPEr1}, ${trimPEr2}"

# 2. Converts intleaved to paired
reformat.sh \
	in=${int} \
	out1=${derepOut1} \
	out2=${derepOut2}

errorExit "reformat.sh failed: ${int}"
echo "reformat.sh finished sucessfully: ${int}"
